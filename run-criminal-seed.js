...
const {
  CriminalEntity
} = require('./dist/features/criminals/entities/criminal.entity');
const {
  ProfileTypeEntity
...
async function runSeed() {
  ...
  const criminalsData = [
    ...
    },
    {
      name: 'Lê Văn C',
      description:
        'Phạm tội cố ý gây thương tích, bị cấm cư trú tại TP.HCM 2 năm',
      birthplace: 'Đà Nẵng',
    ...
    },
    {
      name: 'Phạm Thị D',
      description:
        'Phạm tội lừa đảo chiếm đoạt tài sản, đang chấp hành án quản chế 2 năm',
      birthplace: 'Cần Thơ',
    ...
    },
    {
      name: 'Hoàng Văn E',
      description:
        'Phạm tội giết người, được hoãn chấp hành án phạt tù do lý do sức khỏe',
  ...
  for (const criminalData of criminalsData) {
    ...
    console.log(
      `Criminal created with ${
        criminalProfileTypes.length
      } profile types: ${criminalProfileTypes.map((pt) => pt.name).join(', ')}`
    );
  }
